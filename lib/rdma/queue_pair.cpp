...
namespace faas::rdma {
    ...
    QueuePair::QueuePair(const Infinity *infinity, bool used_shared_cq) : infinity_(infinity), wc_(nullptr) {
        ...
        INFINITY_ASSERT(ibv_recv_completion_queue_ != nullptr, "infinity: Could not allocate recv completion queue")
    ...
    uint64_t QueuePair::PostRequest(WorkRequest &wr, ibv_wr_opcode opcode, uint64_t add) {
        ...
        INFINITY_ASSERT(ret == 0, "infinity: posting read request failed")
    ...
    void QueuePair::NotifyCompletion(CompleteQueue queue) {
        int ret, cnt = 0;
        ...
        for (; ibv_poll_cq(ev_cq, 1, &wc) > 0; cnt++) {
            if (wc.status == IBV_WC_SUCCESS) {
                sprintf(info, "wr_id = %lu, wc_status = %x", wc.wr_id, wc.status);
                INFINITY_DEBUG(info)
            } else {
    ...
    void QueuePair::StateFromRTRToRTS() {
        ...
        INFINITY_ASSERT(ret == 0, "infinity: Cannot transfer to RTS state")
