...
int faas_func_call(void *worker_handle,
    ...
    int ret = context->invoke_func_fn(context->caller_context,
                                      ...
                                      &fn_e_output, &fn_e_output_length, PassingMethod::Fabric);
    ...
    char fn_d_output[60];
    sprintf(fn_d_output, "Via Fabric: interaction latency : %ld\n", end_time - start_time);

    // send the output back to the engine
    context->append_output_fn(context->caller_context, fn_d_output, strlen(fn_d_output));
